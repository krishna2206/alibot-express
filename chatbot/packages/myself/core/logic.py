...
def create_product_elements(current_page_list, **kwargs):
    ...
    for p in current_page_list:
        ...
        title = f"{p.get('title')[:65]}..." if len(p.get('title')) > 65 else p.get('title')
...
def create_product_variant_elements(current_page_list, **kwargs):
    ...
    for variant in current_page_list:
        title = (
            f"{variant.get('variantName')[:65]}..."
            if len(variant.get('variantName')) > 65
            else variant.get('variantName'))
        subtitle = (
            "{}   {}   {}\n".format(
        ...
        # ? Add product to cart
        # ? Choose the number to add if the available quantity is over 1
        if variant.get('availQuantity') > 1:
            target_action = "ask_quantity_to_add"
            payload = Payload(
                target_action=target_action,
...
def create_cart_product_elements(current_page_list, **kwargs):
    ...
    for cart_product in current_page_list:
        ...
        title = f"{product.get('title')[:65]}..." if len(product.get('title')) > 65 else product.get('title')
